...
export const selectStarterTemplate = async (options: { message: string; model: string; provider: ProviderInfo }) => {
  try {
    ...
    if (!response.ok) {
      console.warn('Starter template selection failed with status:', response.status);
    ...
    console.warn('Error in selectStarterTemplate, fallback to blank template:', error);
    return {
      template: 'blank',
      title: '',
    };
...
GenUI Studio está inicializando tu proyecto con los archivos requeridos usando la plantilla ${template.name}.
