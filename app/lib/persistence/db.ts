...
export async function setMessages(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Determine current user ID if not explicitly provided
    let activeUserId = userId;
    if (!activeUserId && typeof window !== 'undefined') {
      try {
        const raw =
          localStorage.getItem('genui_saas_state_v3') ||
          localStorage.getItem('genui_saas_state_v2') ||
          localStorage.getItem('genui_saas_state_v1');
        if (raw) {
          const parsed = JSON.parse(raw);
          activeUserId = parsed.isAuthenticated && parsed.currentUserId ? parsed.currentUserId : 'guest';
        } else {
          activeUserId = 'guest';
        }
      } catch {
