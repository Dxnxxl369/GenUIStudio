...
export function SupabaseConnection() {
  ...
  return (
    <div className="relative">
      <div className="flex rounded-lg overflow-hidden mr-2 text-xs">
        <button
          type="button"
          disabled={connecting}
          onClick={() => setIsDialogOpen(!isDialogOpen)}
          title={isConnected ? `Supabase: ${supabaseConn.project?.name || 'Conectado'}` : 'Conectar con Supabase'}
          className={classNames(
            'flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer font-medium shadow-xs select-none',
            isConnected
              ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-800/60 hover:bg-emerald-100 dark:hover:bg-emerald-900/40'
              : 'bg-zinc-100 hover:bg-zinc-200/80 text-zinc-700 border-zinc-200 dark:bg-[#1a1a17] dark:hover:bg-[#22221e] dark:text-[#f2f0e9] dark:border-[#2a2a25]',
          )}
        >
          <svg className="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none">
            <path
              d="M21.362 9.354H12V.348a.348.348 0 0 0-.586-.253L.784 10.748a.7.7 0 0 0 .49 1.198H12v9.006a.348.348 0 0 0 .586.253l10.63-10.653a.7.7 0 0 0-.49-1.198z"
              fill="#3ECF8E"
            />
          </svg>
          <span className="text-xs max-w-[110px] truncate font-medium">
            {isConnected && supabaseConn.project ? supabaseConn.project.name : 'Supabase'}
          </span>
          <span
            className={classNames(
